...
import com.example.jetflix.presentation.widget.VerticalStaggeredGrid
...
fun FilterRadioItem(title: String, selected: Boolean, onClick: () -> Unit) {
    Row(
        ...
        val color =
            if (selected) MaterialTheme.colors.secondary else MaterialTheme.colors.onSurface.copy(
                alpha = 0.6f
            )
